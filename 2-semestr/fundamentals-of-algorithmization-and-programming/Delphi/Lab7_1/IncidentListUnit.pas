...
type
    ...
    procedure AddTop(Dest, TopValue: Word);
...
Procedure InsertNewTop(Header: PTop; NewTopValue: Word);
Var
    Curr, Temp: PTop;
Begin
    Curr := Header;
...
begin
    ...
    InsertNewTop(Top^.NextTop, TopValue);

    if Dest <> TopValue then
    begin
        Top := FindBaseTopByValue(TopValue);
        InsertNewTop(Top^.NextTop, Dest);
    end;
end;
...
begin
    ...
    if SelectedBaseTop <> SelectedTop then
    begin
        Curr := FindBaseTopByValue(SelectedTop);
        DeleteSecondaryTop(Curr^.NextTop, SelectedBaseTop);
    end;
end;
end.
