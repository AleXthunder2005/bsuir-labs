...
begin
    with AddTopForm do
        ConfirmButton.Enabled := (TopValueEdit.Text <> '') and (WayLengthEdit.Text <> '');
end;

procedure TAddTopForm.EditChange(Sender: TObject);
var
    TempStr:String;
    IValue, CursPos, Max: Integer;
begin
    ...
    Begin
       if Text = '0' then
            ...
            begin
                ...
                if TEdit(ActiveControl).Name = 'WayLengthEdit' then
                    Max := 99
                else
                    Max := CommonBaseTopCount;


                if not TryStrToInt(TempStr, IValue) or (IValue < 1) or (IValue > Max) then
                begin
...
procedure TAddTopForm.EditKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
case key of
        kDOWN:
        ...
        kINSERT:
            Key := 0;
        kENTER: if ConfirmButton.Enabled then ConfirmButton.Click;
        kESC: Close;
    end;
end;

procedure TAddTopForm.EditKeyPress(Sender: TObject; var Key: Char);
begin
