...
Token *getTokenFlowFromFile(char *filePath) {
    int i = 0;
    ...
    arr = (Token *) malloc(sizeof(Token) * 256);

    while (!feof(fInput)) {
        fscanf(fInput, "%s|%s|%d", temp1, temp2, &temp3);
...
Node *errorToken() {
    ...
    printf("TokenType: %s. ValueInSource: %s. Line: %d", convertTokenToString(next->type), next->inSource, next->line);
    printTree(tree, 0);
    return NULL;
}
...
Node *term(const TokenType expected) {
    ...
    if (expected == (next->type)) {
        t = createNode(convertTokenToString(next->type), next->inSource, 0);
    }

    if (next->type != ttPROGRAM_END) {
        next++;
    }
...
Node *statementSet() {
    ...
    next = save;
    if ((t1 = statement()) && (t2 = term(ttNEW_LINE)) && (t3 = statementSet())) {
        new = createNode("StatementList", NULL, 3);
        new->nodes[0] = t1;
        new->nodes[1] = t2;
        new->nodes[2] = t3;
        return new;
    ...
    return createNode("StatementList", NULL, 0);
