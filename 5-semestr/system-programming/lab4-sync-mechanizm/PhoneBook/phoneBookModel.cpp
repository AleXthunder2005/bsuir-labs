...
LRESULT handleCommand(HWND hwnd, WPARAM wParam, LPARAM lParam) {
    ...
    case IDM_FILE_RUN_PARALLEL_SEARCH:
        handleRunParallelSearch(hwnd);
        break;
...
void handleRunParallelSearch(HWND hwnd) {
    std::wstring queryPath = OpenDatabaseFileDialog(hwnd);
    if (queryPath.empty()) {
        return; // Пользователь отменил выбор файла запроса
    }

    std::wstring reportPath = SaveDatabaseFileDialog(hwnd);
    if (reportPath.empty()) {
        return; // Пользователь отменил выбор файла для сохранения отчета
    }

    if (RunParallelSearchFromFile(queryPath.c_str(), reportPath.c_str())) {
        MessageBoxW(
            hwnd,
            L"Параллельный поиск успешно выполнен!",
            L"Поиск",
            MB_OK | MB_ICONINFORMATION
        );
    }
