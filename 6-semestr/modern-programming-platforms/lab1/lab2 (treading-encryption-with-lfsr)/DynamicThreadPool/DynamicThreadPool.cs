...
public sealed class DynamicThreadPool<T> : IDisposable
{
    ...
    public TimeSpan IdleTimeout { get; } // таймаут простоя безработного потока
    public TimeSpan QueuePressureTimeout { get; } // таймаут давления на очередь после которого надо создавать воркера
    public TimeSpan HungWorkerTimeout { get; } // таймаут зависшего потока
    public TimeSpan SupervisorInterval { get; } // интервал проверки состояния пула
    ...
    public PoolHandle<T> Enqueue(Func<T> work, int priority = 0, string? name = null)
    {
        ...
        // Когда задача поставлена в очередь
        ...
        try
        {
            while (true)
            {
                ...
                lock (_sync)
                {
                    ...
                    // если воркер надо завершить - завершаем
